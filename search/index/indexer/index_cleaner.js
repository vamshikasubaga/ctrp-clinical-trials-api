...
class IndexCleaner extends AbstractIndexTool {
  ...
  /**
   * Initializes and executes the cleaning of old trial and term indices
   * 
   * This will remove all indicies greater than 'daysToKeep' old that 
   * begin with the alias names supplied index the trialIndexInfo 
   * and termIndexInfo objects.  NOTE: This will not remove indicies associated
   * with either alias regardless of age.  
   * 
   * @static
   * @param {any} adapter The search adapter to use for making requests to ElasticSearch
   * @param {any} trialAlias The alias name for clinical trials
   * @param {any} termAlias The alias name for the terms
   * @param {any} daysToKeep The number of days of indices to keep.
   * @param {any} callback
   */
  static init(adapter, trialAlias, termAlias, daysToKeep, callback) {

    let cleaner = new IndexCleaner(adapter);
    cleaner.logger.info(`Starting index cleaning.`);
    
    async.waterfall([
      (next) => { cleaner.cleanIndicesForAlias(trialAlias, daysToKeep, next); },
      (next) => { cleaner.cleanIndicesForAlias(termAlias, daysToKeep, next); }
    ], (err) => {
